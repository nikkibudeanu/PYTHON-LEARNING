...
"""
Define a function named ten_rand_nums (that takes no parameters) and that returns a list of 10 random integers 
between 0 and 100
"""
import random 
...
"""
In addition to the built-in libraries, as Python is open source, 
anyone can create software and share it with the community. 
...
# In the runnable example, we are using a third-party library named numpy. 
# It is a widely used library for scientific computing. 
# We can give the package name an alias (np in this case), so we don’t have to type it out each time.
#  Numpy extends the abilities of Python into scientific computing. Here we are creating a simple one-dimensional array. 
# The repl.it already has numpy installed. 
# If you were trying this on your computer, you would have to install numpy locally as it is a third-party library 
...
One example is simple statements. An expression statement can be used to compute and write a value. 
You will also have seen statements in the terminal when an error occurs. 
These are assertion and raise statements. 
We will see more of those in the upcoming error handling units. 
In the io unit, you saw that output could also write to a file. 
We will cover this in more detail in an upcoming unit. 
The most frequently used output you have seen is the print function. 
This allows you to output data in a human-readable format.


Let’s look at fancier ways to format our output. 
The most basic output is a statement. 
In this case, we have a simple one-line expression (10 + 20) which is assigned to a variable i. 
...
If instead of a format specifier an integer is placed after the colon, 
the field will be set to a minimum character value equal to the integer value. 
Here we have chosen 25 to take into account the longest string.
...
"""
The most uncomplicated persistent storage is a text file. 
You can save lines of text in the file then use python io to open the file and read the contents. 
There are several methods to do this. You can use readlines() which reads all the lines into a list, 
readline() which reads one line at a time, read() which reads the entire file or seek() 
which moves to a particular point in a file.

It’s important to understand that when you open a file, 
you are reading or writing to a particular position within the file. 
...
It’s also important to understand that files don’t have lines. 
...
# In the runnable example, the code opens the file, reads the lines it contains into a variable called lines. 
# When printed out, we see that it's a list of strings. 
# The 'r means that the file is opened as read-only. 
# We can also see that each line contains the newline character '\n'. 
# This makes sense because the file data.txt contains four separate lines. 
# What is interesting is that the readlines method splits the data read from the files at those newline characters, 
# to create a line of strings. Modify the program slightly and use the read method instead of readlines. 
# All of the data will be read into a single string, including the newline characters. 
...
""" 
A straightforward, practical thing we can do with text files is to count the occurrences of words. 
Here's a runnable example. It uses two standard libraries, re and collections . 
The re library will allow us to identify the words in the file using regular expressions, and the 
collections library allows us to count occurrences of words. 
Notice that the entire contents of the file are read into a single string called text , and the findall method 
is then used to parse that string and find words. A full discussion of regular expressions is beyond the scope 
of this lesson; however, the findall method ensures that all occurrences of the pattern are found. 
The pattern for a word is ' \w+ '. The ' \w ' denotes 'not whitespace'. The ' + ' denotes 'one or more'. 
So, every occurrence of one or more characters that are not whitespace will be considered a word. 
It's not perfect, it may get some false positives, but it's fine for our purposes. 
The Counter method of collections will count how many occurrences there are of each word. 
The most_common method will limit the results to the top 10 words.
 When this is printed, we get a list of tuples where each tuple contains a word and a number of occurrences.
"""
...
There are many specific exceptions in Python; for example, ZeroDivisionError raised when the second argument of 
a division or modulo operation is zero. However, if an error does not fall into one of these specific categories, 
a RuntimeError will be raised. The associated string will explain what has gone wrong.

In the runnable example, the code will print text to the terminal if 1 or 2 is entered. 
...
A RuntimeError is posted in the terminal, but it is not particularly informative. 
Also, our program has still crashed. In the next units, we see how to handle these errors 
without crashing the program and how to get a more informative error message.
...
# try except
"""
In the last example, we handled the error by telling the user that they had not entered a number. 
...
If we return to the previous example, we have added ValueError to the except statement. 
This is one of the built-in exceptions in Python. It is more specific than RuntimeError, 
and we have used it as we know in this case, an inappropriate value input causes an error. 
...
In the runnable example, we have three except blocks. 
If more than one is valid, the first exception raised is the one you’ll see. 
Here we check for division by zero and an inappropriate value entered. Try both to see what happens.
If another error occurs that the first two except blocks don’t catch then the third block catches it.
To test this, you can provoke an error by trying ctrl-c in the terminal.
...
"""
In the runnable example below there is no file book.txt, so the except block runs. 
...
This is placed after all of the except clauses and deals with any code that must be executed in the case 
where the try clause does not raise an exception. 
...
It runs whether or not an exception has been raised. It is intended for any ‘clean-up’ code 
that must run regardless of whether an exception was caught or not.


In the runnable example, we have a function that opens a text file, counts the lines and prints the opening 
line of the file. 
The try block catches if a file does not exist and the except block deals with the error so that the
 ...
 Here it just lets the user know that the function has complete. 
 Note that it runs after the try, except and else blocks.
